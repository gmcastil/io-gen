...
_DEFAULT_CONSTRAINTS = {"config_voltage": 3.3, "cfgbvs": "VCCO"}
...
def test_generate_xdc_returns_str() -> None:
    ...
    assert isinstance(generate_xdc(st, pt, ct), str)


def test_xdc_header_present() -> None:
    """Output contains the 'Generated by io-gen' header."""
    st, pt, ct = _make_tables(
        [
            {
                "name": "sys_clk",
    ...
    assert "# Generated by io-gen" in generate_xdc(st, pt, ct)
...
def test_port_name_in_output(sig: dict, expected_port: str) -> None:
    ...
    st, pt, ct = _make_tables([sig])
    assert expected_port in generate_xdc(st, pt, ct)
...
def test_direction_present_in_pin_planner_mode(sig: dict, expected_line: str) -> None:
    """A DIRECTION constraint is emitted for every port when pin_planner=True."""
    st, pt, ct = _make_tables([sig])
    assert expected_line in generate_xdc(st, pt, ct, pin_planner=True)
...
def test_direction_absent_in_rtl_mode(sig: dict, expected_line: str) -> None:
    """No DIRECTION constraint is emitted when pin_planner=False (default)."""
    st, pt, ct = _make_tables([sig])
    assert expected_line not in generate_xdc(st, pt, ct)
...
def test_xdc_comment_emitted() -> None:
    ...
    assert "# 125 MHz system clock" in generate_xdc(st, pt, ct)


def test_no_comment_produces_no_hash_line() -> None:
    """A signal with no comment.xdc produces no # line in the signal block."""
    st, pt, ct = _make_tables(
        [
            {
                "name": "sys_clk",
    ...
    # Skip the header block; only the signal blocks should be checked
    signal_section = generate_xdc(st, pt, ct).split("\n\n", 1)[1]
    assert "#" not in signal_section
...
def test_bypass_true_included() -> None:
    ...
    assert "spare_pad" in generate_xdc(st, pt, ct)


_INTEGRATION_DOC = {
    "title": "Test",
    "part": "xc7k325tffg900-2",
    "constraints": {"config_voltage": 3.3, "cfgbvs": "VCCO"},
    "signals": [
        {
            "name": "sys_clk",
            "pins": "G22",
...
def test_integration_output_pin_planner() -> None:
    """Full document produces the expected XDC string in pin planner mode."""
    st = build_signal_table(_INTEGRATION_DOC)
    pt = build_pin_table(st)
    ct = build_constraints_table(_INTEGRATION_DOC)
    assert generate_xdc(st, pt, ct, pin_planner=True) == _EXPECTED_XDC_PIN_PLANNER


def test_integration_output_rtl() -> None:
    """Full document produces the expected XDC string in RTL mode (no DIRECTION)."""
    st = build_signal_table(_INTEGRATION_DOC)
    pt = build_pin_table(st)
    ct = build_constraints_table(_INTEGRATION_DOC)
    assert generate_xdc(st, pt, ct) == _EXPECTED_XDC_RTL
...
def test_generate_xdc_ends_with_newline() -> None:
    ...
    assert generate_xdc(st, pt, ct).endswith("\n")
