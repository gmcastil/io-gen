...
def test_signal_table_row_shape(sig: dict, expected_row: dict) -> None:
    """Each signal produces a fully resolved row with all expected fields."""
    table = SignalTable()
    table.add(sig)
    rows = list(table)
    assert len(rows) == 1
...
def test_build_signal_table_count_and_order() -> None:
    """build_signal_table produces one row per signal in YAML order, skipping generate:false."""
    doc = {
        "title": "Test",
